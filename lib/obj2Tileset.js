...
function obj2Tileset(objPath, outputpath, options, iscombined) {
    ...
    // var folder = path.dirname(outputpath);
    // var tileFullName = path.basename(outputpath);
    // //var folderPrifix = options.b3dm ? 'Batched_' : 'Instanced_';
    // // var tilesetFolderName = folderPrifix + path.basename(objPath, '.obj');
    // var tilesetFolderName = path.basename(objPath, '.obj');
    // var tilePath = path.join(folder, tilesetFolderName, tileFullName);
    // var tilesetPath = path.join(folder, tilesetFolderName, 'tileset.json');
    // var tilesetOptions = options.tilesetOptions || {};
    if (options.b3dm) {
            ...
            .then(function (result) {
                ...
                return new Promise(function (resolve) {
                    ...
                    return resolve({
                        ...
                        tilesetJson: createSingleTileset(tilesetOptions, objPath, iscombined),
                        tilePath: tilePath,
                        tilesetPath: tilesetPath,
                        tilesetFolderName: tilesetFolderName
                    });
                });
            });
            ...
            .then(function (result) {
                ...
                return new Promise(function (resolve) {
                    ...
                    return resolve({
                        i3dm: result.i3dm,
                        batchTableJson: result.batchTableJson,
                        tilesetJson: createSingleTileset(tilesetOptions, objPath, combine),
                        tilePath: tilePath,
                        tilesetPath: tilesetPath
                    });
                });
