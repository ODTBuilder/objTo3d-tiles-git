 ...
 * 
 */
function createSingleTileset(options, objPath, iscombined) {
    ...
    var upAxis = defaultValue(options.gltfUpAxis, 'Z');
    ...
    if (options.region) {
        ...
        var defultwest = longitude - longitudeExtent / 2 + offsetX / tileWidth * longitudeExtent;
        var defultsouth = latitude - latitudeExtent / 2 - offsetY / tileHeight * latitudeExtent;
        var defulteast = longitude + longitudeExtent / 2 + offsetX / tileWidth * longitudeExtent;
        var defultnorth = latitude + latitudeExtent / 2 - offsetY / tileHeight * latitudeExtent;

        var west = defaultValue(options.west, defultwest);
        var south = defaultValue(options.south, defultsouth);
        var east = defaultValue(options.east, defulteast);
        var north = defaultValue(options.north, defultnorth);
    ...
    var degreex = longitude * (180 / pi);
    var degreey = latitude * (180 / pi);
    // obj path
    var strArr = objPath.split("/");
    var originObj;
    if (iscombined) {
        originObj = strArr[3] + "/obj/" + strArr[4] + "/" + strArr[5];
    } else {
        originObj = strArr[3] + "/obj/" + strArr[4];
    }
